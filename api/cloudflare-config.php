...
$requiredEnv = ['CLOUDFLARE_ACCOUNT_ID', 'CLOUDFLARE_ACCESS_KEY', 'CLOUDFLARE_SECRET_KEY'];
foreach ($requiredEnv as $env) {
    if (!getenv($env)) {
        error_log("ERREUR: Variable d'environnement $env manquante");
    }
}

// ID du compte Cloudflare (Account ID)
...
// ===============================
// Clés S3 (PAS le token API)
// ===============================
define('CLOUDFLARE_ACCESS_KEY', getenv('CLOUDFLARE_ACCESS_KEY'));
define('CLOUDFLARE_SECRET_KEY', getenv('CLOUDFLARE_SECRET_KEY'));
...
// Limites
// ===============================
define('CLOUDFLARE_MAX_FILE_SIZE', 500 * 1024 * 1024); // 500 MB
...
function generateCloudflareUrl($objectKey) {
    ...
    // Générer l'URL publique : l'URL de base + la clé
    // Ne pas ajouter le nom du bucket car l'URL publique pointe déjà à la racine du bucket
    return CLOUDFLARE_PUBLIC_URL . '/' . ltrim($objectKey, '/');
}
...
function uploadToCloudflareR2($filePath, $objectKey, $contentType = null) {
    try {
        ...
        // Upload vers R2 (sans ACL car non supporté par R2)
...
function extractObjectKeyFromUrl($url) {
    ...
    // Si l'URL contient le nom du bucket, le retirer (pour compatibilité)
    if (strpos($path, CLOUDFLARE_BUCKET . '/') === 0) {
        return substr($path, strlen(CLOUDFLARE_BUCKET) + 1);
    }
    
    // Sinon, retourner le chemin complet (qui est la clé)
    return $path;
}
